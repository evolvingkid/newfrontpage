...
//print(lists);
  ...
  List selectCategory() {
    for (int i = 0; i < fetchData[0][0]['table_menu_list'].length; i++) {
      ...
      catergoryData.insert(
          i, fetchData[0][0]['table_menu_list'][i]['menu_category']);

      listWdgets.add(Padding(
        padding: const EdgeInsets.all(8.0),
        child: new Text(
          fetchData[0][0]['table_menu_list'][i]['menu_category'],
          //style: TextStyle(color: Colors.black),
        ),
      ));

      bottomListWdgets.add(ItemListViews(
...
//print('cat: $catergoryData');
  ...
  itemDatas() {
    //print(fetchData[0][0]['table_menu_list'][0]['category_dishes'][0]['addoncat']);
    //print('dish : ${fetchData[0][0]['table_menu_list'][0]['category_dishes'][0]['dish_name']}');

    for (int i = 0; i < fetchData[0][0]['table_menu_list'].length; i++) {
      
      for (int j = 0;
          j < fetchData[0][0]['table_menu_list'][i]['category_dishes'].length;
          j++) {

        // print('dish : ${fetchData[0][0]['table_menu_list'][i]['category_dishes'][j]['dish_name']}');

       // print(
            //'cards: ${fetchData[0][0]['table_menu_list'][i]['category_dishes'][j]['addonCat']}');
        dish.add({
          'category':
              '${fetchData[0][0]['table_menu_list'][i]['menu_category']}',
          'dishname':
              '${fetchData[0][0]['table_menu_list'][i]['category_dishes'][j]['dish_name']}',
          'dishImg':
              '${fetchData[0][0]['table_menu_list'][i]['category_dishes'][j]['dish_image']}',
          'dishprice':
              '${fetchData[0][0]['table_menu_list'][i]['category_dishes'][j]['dish_price']}',
          'dish_currency':
              '${fetchData[0][0]['table_menu_list'][i]['category_dishes'][j]['dish_currency']}',
          'dish_calories':
              '${fetchData[0][0]['table_menu_list'][i]['category_dishes'][j]['dish_calories']}',
          'dishDesc':
              '${fetchData[0][0]['table_menu_list'][i]['category_dishes'][j]['dish_description']}',
          'dishAvailibility':
              '${fetchData[0][0]['table_menu_list'][i]['category_dishes'][j]['dish_Availability']}',
          'dishtype':
              '${fetchData[0][0]['table_menu_list'][i]['category_dishes'][j]['dish_Type']}',
          'addoncat':
              '${fetchData[0][0]['table_menu_list'][i]['category_dishes'][j]['addonCat']}',
        });
      }
    }
    // print('whole Dish: ${dish[0]['dishImg']}');
  }
